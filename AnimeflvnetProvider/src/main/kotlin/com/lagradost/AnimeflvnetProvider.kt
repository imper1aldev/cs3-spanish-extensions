...
class AnimeflvnetProvider : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        ...
        Pair("$mainUrl/browse?type[]=movie&order=updated&page=", "Películas"),
        Pair("$mainUrl/browse?status[]=2&order=default&page=", "Animes"),
        Pair("$mainUrl/browse?status[]=1&order=rating&page=", "En emisión"),
    )
    ...
    override suspend fun getMainPage(page: Int, request : MainPageRequest): HomePageResponse {
        ...
        if (request.data == mainUrl) {
            ...
            val homePage = HomePageList(
                request.name,
                soup.select("ul.ListAnimes li article").mapNotNull {
        ...
        app.get(data).document.select("script").apmap { script ->
            if (script.data().contains("var videos = {") || script.data().contains("var anime_id =") || script.data().contains("server")) 
            {
                ...
                fetchUrls(videos).map {
                    it.replace("https://embedsb.com/e/", "https://watchsb.com/e/").replace("https://ok.ru", "http://ok.ru")
                }.apmap {
