...
class CinecalidadProvider : MainAPI() {
    ...
    override suspend fun getMainPage(
        ...
        val hasNextPage = soup.select("a.nextpostslink").any()
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val description = soup.selectFirst("div.single_left table tbody tr td p:nth-child(4)")?.text()?.trim()
    ...
    override suspend fun loadLinks(
        ...
        val domainUrl = domainRegex.findAll(datam.url).firstOrNull()?.value ?: "https://cinecalidad.lol"

        doc.select(".dooplay_player_option").apmap {
            val url = it.attr("data-option")
            loadExtractor(url, mainUrl, subtitleCallback, callback)
            if (url.startsWith(domainUrl)) {
                app.get(url).document.select("iframe").map { frame ->
                    val extractedUrl = frame.attr("src")
...
//                }
                ...
                if (url.startsWith(domainUrl)) {
                    val cineurlregex =
                        Regex("(https:\\/\\/cinecalidad\\.run\\/play\\/\\?h=[a-zA-Z0-9]{0,8}[a-zA-Z0-9_-]+)")
                    cineurlregex.findAll(url).map {
                        ...
                        app.get(
                            it,
                            headers = mapOf(
                                "Host" to domainUrl.substringAfter("https://"),
