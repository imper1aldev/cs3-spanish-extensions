...
class EstrenosDoramasProvider : MainAPI() {
    ...
    override var mainUrl = "https://www24.estrenosdoramas.net/"
    ...
    override val mainPage = mainPageOf(
        ...
        Pair("$mainUrl/category/doramas-online/page/", "Doramas"),
        Pair("$mainUrl/category/peliculas/page/", "Películas"),
    )

    override suspend fun getMainPage(page: Int, request : MainPageRequest): HomePageResponse {
        val items = ArrayList<HomePageList>()
        val link = if(request.data == mainUrl) request.data else request.data + page
        val soup = app.get(link, timeout = 120).document
        val hasNextPage = soup.select("a.nextpostslink").any()
        val home = soup.select("div.clearfix").map {
            val title = cleanTitle(it.selectFirst("h3 a")?.text()!!)
            val poster = it.selectFirst("a img.cate_thumb")?.attr("src")
            AnimeSearchResponse(
                title,
